...
func TestCompileQuery(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			"[asof (precedes) [scan (a device all-time) [binexp [= a.b 20]]] [scan (b device all-time) [binexp [= b.c 10]]]]",
		},
		{
			"asof join with restriction",
			"from device a precedes b by less than 5 seconds",
			"[asof (precedes seconds 5) [scan (a device all-time)] [scan (b device all-time)]]",
		},
	}
	parser := ql.NewParser()
	for _, c := range cases {
